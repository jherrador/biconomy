...
const Welcome = () => {
  const { connectWallet, currentAccount, formData, stake, handleChange } = useContext(LockedVaultContext)
      ...
      <div className='flex mf:flex-row flex-col items-start justify-betewen md:p-20 py-20 py-12 px-4'>
        <div className="flex flex-1 justify-start flex-col mf:mr-10">
          ...
          <p className='text-left mt-5 text-white font-light md:w-9/12 w-11/12 text-base'>
            Explore the crypto worl. Buy and sell crypto easily
          </p>
          {!currentAccount && (
            <button
            type="button"
        ...
        <div className='flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10'>
          ...
          <select className='my-2 w-full rounded-sm p-2 outline-none bg-transparent text-white border-none text-sm white-glassmorphism'>
            {['Eth', 'StandardToken'].map((element) => (
              <option value={element}>{element}</option>
            ))}
          </select>
            <Input placeholder="TokenAddress" name="TokenAddress" type="text" handleChange={handleChange}/>
